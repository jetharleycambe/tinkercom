<?php
/**
 * PART 4 — REFACTORED PRINTERS PAGE
 * FILE: printers.php  (REPLACE your existing printers.php entirely)
 *
 * What's new:
 *  - Left sidebar with sub-category filter (Inkjet / All-in-One / Laser)
 *  - Brand filter, price range, sort — all in one clean filter bar
 *  - Uses new product-card layout (same as homepage Top Sellers)
...
// ── Sub-categories under Printers (parent_id = 1) ──────────
$subs_result = mysqli_query($conn,
    "SELECT category_id, category_name FROM categories
     WHERE parent_category_id = 1 AND is_visible = 1
...
// ── Build WHERE clause ──────────────────────────────────────
// If a sub-category is selected, filter by that sub-cat ID
// Otherwise show all sub-categories of Printers (parent_id = 1)
if ($sub_id > 0) {
    $cat_where = "p.category_id = $sub_id";
} else {
    // All sub-cats under Printers
...
// ── Brands available in this section ───────────────────────
...
$brands_result = mysqli_query($conn,
    "SELECT DISTINCT TRIM(brand) AS brand
     FROM products
     WHERE category_id IN ($sub_ids_str_for_brands)
       AND is_visible = 1
     ORDER BY brand ASC"
);

// ── Current sub-category name (for page heading) ───────────
$current_sub_name = 'All Printers';
if ($sub_id > 0) {
...
<div class="listing-page">
    ...
    <aside class="listing-sidebar">
        ...
        <div class="sidebar-section">
            <h3 class="sidebar-title">Printer Type</h3>
            <ul class="sidebar-links">
                ...
                <li>
                    <a href="printers.php?sub=<?php echo $s['category_id'];
        ...
        <div class="sidebar-section">
            ...
            <ul class="sidebar-links">
                <li>
                    <a href="printers.php?<?php echo $sub_id ? 'sub='.$sub_id.'&' : ''; ?>sort=<?php echo $sort; ?>"
                ...
                <li>
                    <a href="printers.php?<?php echo $sub_id ? 'sub='.$sub_id.'&' : ''; ?>brand=<?php echo urlencode($b['brand']); ?>&sort=<?php echo $sort; ?>"
        ...
        <div class="sidebar-section">
            ...
            <form method="GET" action="printers.php" class="sidebar-price-form">
                ...
                <?php if ($min_price > 0 || $max_price < 999999): ?>
                    <a href="printers.php?<?php echo $sub_id ? 'sub='.$sub_id.'&' : ''; ?><?php echo $brand_filter ? 'brand='.urlencode($brand_filter).'&' : ''; ?>sort=<?php echo $sort; ?>"
    ...
    <main class="listing-main">
        ...
        <div class="listing-topbar">
            <h1 class="listing-title">
                ...
                <span class="listing-count">(<?php echo $total; ?> product<?php echo $total !== 1 ? 's' : ''; ?>)</span>
            </h1>

            <form method="GET" action="printers.php" class="sort-form">
                <?php if ($sub_id): ?><input type="hidden" name="sub" value="<?php echo $sub_id; ?>"><?php endif; ?>
                <?php if ($brand_filter): ?><input type="hidden" name="brand" value="<?php echo htmlspecialchars($brand_filter); ?>"><?php endif; ?>
        ...
        <div class="active-filters">
            ...
            <?php if ($brand_filter): ?>
                <span class="filter-tag">
                    ...
                    <a href="printers.php?<?php echo $sub_id ? 'sub='.$sub_id.'&' : ''; ?>sort=<?php echo $sort; ?>">✕</a>
                </span>
            <?php endif; ?>
            <?php if ($min_price > 0 || $max_price < 999999): ?>
                <span class="filter-tag">
                    Price: ₱<?php echo number_format($min_price); ?> – <?php echo $max_price < 999999 ? '₱'.number_format($max_price) : 'Any'; ?>
                    <a href="printers.php?<?php echo $sub_id ? 'sub='.$sub_id.'&' : ''; ?><?php echo $brand_filter ? 'brand='.urlencode($brand_filter).'&' : ''; ?>sort=<?php echo $sort; ?>">✕</a>
                </span>
            <?php endif; ?>
        </div>
        <?php endif; ?>
        ...
        <div class="listing-empty">
            ...
            <p class="empty-sub">Try adjusting your filters or <a href="printers.php">clear all filters</a>.</p>
        </div>
        <?php endif; ?>
